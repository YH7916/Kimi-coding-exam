...
def _best_snippet_for_terms(document: Document, terms: list[str]) -> str:
    ...
    for term in terms:
        if _casefold(term) in _casefold(document.text) or _casefold(term) in _casefold(
            document.title
        ):
...
def semantic_search(
    ...
    """Search documents by deterministic semantic expansion and ranking."""
    ...
    for document in documents:
        ...
        for term in terms:
            title_matches = _count_occurrences(document.title, term)
            text_matches = _count_occurrences(document.text, term)
            score += title_matches * 3.0 + text_matches * 0.7
        if score <= 0:
            continue
        results.append(
            SearchResult(
